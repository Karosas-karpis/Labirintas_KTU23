...
def level_2():
    ...
    Arr1 = np.array(MazeArray)
    MultiplicationValue = 2.15
    # Kiekvieno elemento daugyba
    LargeMazeArray = [[element * MultiplicationValue for element in row] for row in MazeArray]
    

    # Labirinto nupaišymas naudojant kordinates
    for i in range(len(LargeMazeArray)):
        CoordinatesXY = LargeMazeArray[i]
        l.create_line(CoordinatesXY ,fill="black",width=5) 
